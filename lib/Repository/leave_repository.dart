...
class LeaveRepository extends BaseApi {
  ...
  Future<ApiResult<LeaveModel>> leaveAction(int id, String state) async {
    Map<String, dynamic> map = await request(
        uri: Endpoint.leaveAction, params: LeaveParams.action(id, state));
    // map = {"status_code": 200};
    return apiResponse(
      status: ApiStatusModel.fromJson(map),
      data: LeaveModel.fromJson(map['data'] ?? {}),
  ...
  Future<ApiResult<LeaveModel>> toApproveList() async {
    Map<String, dynamic> map = await request(
        uri: Endpoint.leaveToApproveList,
        params: {
          "company_ids":
  ...
  Future<ApiResult<InOutModel>> attendanceList({
    required int page,
    required String from,
    required String to,
  }) async {
    Map<String, dynamic> map = await request(
        uri: Endpoint.attendanceList,
        params: LeaveParams.attendanceList(page, from, to));
    // map = {"status_code": 200};
    return apiResponse(
      status: ApiStatusModel.fromJson(map),
